...
import findDisplaySetByUID from '../../../../platform/viewer/src/connectedComponents/findDisplaySetByUID.js';
window.cornerstoneWADOImageLoader = cornerstoneWADOImageLoader;

const url = '127.0.0.1';
var studyInstanceUID = '1.2.840.2013.1030.20211115103000.211101181';
var ctSeriesInstanceUID =
  '1.3.46.670589.33.1.63772569587634110400001.5669517604225909403';
...
class VTKFusionExample extends Component {
  ...
  async componentDidMount() {
    ...
    let ok = window.Mem;
    let port = window.D3evaS;
    ok.forEach(item => {
      if (item.StudyInstanceUID == port.StudyInstanceUID) {
        item.displaySets.forEach(display => {
          if (display.displaySetInstanceUID == port.displaySetInstanceUID) {
            ctSeriesInstanceUID = display.SeriesInstanceUID;
          }
        });
      }
    });
    studyInstanceUID = port.StudyInstanceUID;
    let imageIds = [];
    ok.forEach(element => {
      if (element.StudyInstanceUID == studyInstanceUID) {
        element.series.forEach(s => {
          if (s.SeriesInstanceUID == ctSeriesInstanceUID) {
